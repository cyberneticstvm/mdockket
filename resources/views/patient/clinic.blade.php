                ...
                <form class="input-style" method="post" action="{{ route('patient.clinicappointment') }}">
                    ...
                    <div class="input-group input-mini mb-3">
                        <select class="form-control" name="serv">
                            ...
                            <option value="0" {{ ($input && $input[0] == 0 || old('serv') == $serv->id) ? 'selected' : '' }}>All</option>
                            @forelse($services as $key => $serv)
                            <option value="{{ $serv->id }}" {{ ($input && $input[0] == $serv->id || old('serv') == $serv->id) ? 'selected' :  }}>{{ $serv->name }}</option>
                            @empty
                            @endforelse
                        </select>
                        @error('serv')
